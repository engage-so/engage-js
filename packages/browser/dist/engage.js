...
(function (global, factory) {
	...
	async function _request (url, params, method) {
	  try {
	    ...
	    const response = await browserPonyfill(url, o);
...
`;
	...
	function onMessage (e) {
	  ...
	  if (data.action === 'send_chat') {
	    ...
	    core.request('/messages/chat', params, 'POST')
	      .then(body => {
	        // Set uid
	        if (!body.uid) {
	          return
...
<img src="https://us-central1-suet-170506.cloudfunctions.net/ppx?s=o&d=${o.domain_id}&m=${o.msg_id}">`;
	  ...
	  socket.on('connect', async () => {
	    if (uid) {
	      ...
	      if (wasDisconnected) {
	        ...
	        try {
	          const chat = await core.request('/messages/chat/open?uid=' + uid + '&since=' + lastMsgId);
	...
	loadMessageFrame(async () => {
	  try {
	    account = await core.request('/account');
	    // Turn on chat icon
	  ...
	  try {
	    const chat = await core.request('/messages/chat/open?uid=' + uid);
	    if (chat && chat.messages && chat.messages.length) {
	  ...
	  try {
	    const msg = await core.request('/messages/push/latest?uid=' + uid);
	    if (msg && msg.body) {
	      const opened = containerDiv.classList.contains('opened');
	      if (!opened) {
	  ...
	  try {
	    const data = await core.request('/campaigns/banners/active?uid=' + uid + '&path=' + window.location.protocol + '//' + window.location.href);
	      ...
	      if (data.style === 'inline') {
	        ...
	        if (data.dismiss) {
	          ...
	          link.addEventListener('click', async (e) => {
	            ...
	            // Track dismiss
	            await core.request('/campaigns/banners/' + data.id + '/track?uid=' + uid);
	          });
	...
	// Button click
	core.openChat = function () {
	  toggleWidget();
	};
	core.openHelp = function (id, helpType, locale) {
	  const help = {
	    id
	  };
	  if (helpType) {
